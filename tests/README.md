...
- **本地绿不等于判据稳（P70）**：Windows 与 CI 的 ubuntu 字体度量 / 可滚动高度不同。
  两条硬规矩：① 自己注入的多个浮层要互相岔开，**视口正中（`scrollIntoView` 的落点）不许放 fixed 浮层**；
  ② hit-test 类断言先断言前置状态（如 `scrollY === 600`）再断言命中，否则是"假绿"。
  仿真用例在 CI 上会实跑（ubuntu 自带 Chrome），是这道防线的唯一交叉验证手段，别绕过它。
...
| `hdhomeui/verify-hdui-real-page.js` | **真站脱敏整页离线渲染验收**（P49 硬要求）。原本只存在于 gitignore 的 `.workbuddy-ai/_verify-hdui-real-render.js` —— 不入库就没人能复现这套验收、也永远进不了回归。现在入库并做成用例：**缺真站样本 / 缺浏览器时 SKIP 退 0**（样本在 `resources-do-not-track/`，已 gitignore），有样本时必须真过。覆盖：仍正常上妆（防新增的卸妆规则误判真站）/ 文档宽不被撑宽 / CSS 无静默失败 / 类别·促销图标全换 SVG（含未登记家族的兜底）/ 标签药丸且保留站点底色 / 置顶行内外两层都有背景 / 五档扫描全无 / color-scheme=dark。**只输出结构·几何·颜色·计数，绝不输出页面文本**（铁律 5） | P49 / P65 |
