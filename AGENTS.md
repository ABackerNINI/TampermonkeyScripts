...
- **协作主线**: 日常开发在 `develop`(2026-09-20 由 `dev` 改名), 统一以 **Gitee 的 `develop`** 为准; **交付与否只看 Gitee 上有没有该提交**。GitHub 只作镜像, **允许滞后** —— 不要用 GitHub 的提交状态判断进度(直连不稳定, 会误判成"改动没推上去")。远端旧的 `dev` 分支保留不动。
