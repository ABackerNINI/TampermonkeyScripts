# TASK018 · HDHome 界面主题（胶片墙）

> ⚠️ **本文件是历史设计档案。** 原计划做「5 套可切换 UI」，那 5 套已于 `2026.09.19.7`
> **全部移除**（用户判定"已经没用了"），改为单一主题**胶片墙**（原型 `.workbuddy-ai/hdui-mock/film.html`）。
> 下文涉及"5 套 / 片库索引 / 电传纸带 / 大开本 / 瑞士网格 / 播控台"的部分**均为历史记录，不代表现状**。
> **现状请看** `memory-bank/scripts/HDHomeUI.md`（尤其是文件头的 `.7` 条目）。
>
> 状态：**已实施**（`.1`–`.4` 已提交；`.5` 图标体系 / `.6` 归因修正 / `.7` 换胶片墙 待审核提交）
> 提出日期：2026-09-19
