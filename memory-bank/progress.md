...
- **HDHomeUI**（`2026.09.19.1`，2026-09-19 新建，TASK018）：HDHome 界面主题套件，5 套可切换可记忆的 UI
...
- **2026-09-18 本地仿真站与安全测试（TASK017）已实施**：`tests/lib/sim/`（零依赖服务器 + 站点剧本 + GM 垫片 + CDP 驱动）建成，
  ...
  **已提交**（`e2d5f52`，分支 `dev`）。
- **分支 `dev` 上共 10 个提交尚未 push**（`ebb06bb` / `2b31377` / `8036f44` / `e2d5f52` / `18e0824` / `0bac142` / `d63fe0c` / `ab9701a` / `80e0f37` / **`13266e0`（HDHomeUI）**，`dev` 无上游跟踪；机械数 `git rev-list --count dev --not origin/master` 现在 = 12，按仓库惯例**不含状态登记提交本身**（`9283295` / `6cd436b`），2026-09-19 复核）。
- 其余脚本功能稳定，处于增量维护状态。
