...
- **2026-09-19 主面板交互（.4，已改未提交）**：用户需求「主弹出 UI 改为失焦自动关闭，同时检测到签到按钮重现时不弹出主 UI」。
