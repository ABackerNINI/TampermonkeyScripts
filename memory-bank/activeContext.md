...
0. **HDHomeUI `.5`~`.10` 已一次性提交 `d56ffbb`**（分支 `dev`，**尚未 push**）：
   图标体系 + 归因修正 + 换胶片墙 + 旧主题迁移 + 首装上妆 + 去撞色 + 布局修复，全量 26/26。
   拆不成多次提交 —— 代码改动集中在 `src/HDHomeUI.user.js`（690 行，各版 hunk 交错）。
   **下一步是真站实测**（仿真只能证明"没回归"，看不出真站观感）：① 首装上妆后的整体观感；
   ② 齿孔/帧号在真实字号下是否还清晰；③ 窄屏（笔记本 1366 及以下）横向滚动是否可接受；
   ④ 与 PTAutoCheckIn 的 FAB 是否还打架（入口已内嵌，理论上不会）。
