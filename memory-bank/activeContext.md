...
**2026-09-19 `.25`（已改码，**待审核**）——第五轮用户实拍：dock / 嵌套置顶 / 搜索箱（P65）：**
① **切换按钮不明显**：`.dock` 原来 background opacity 仅 `.045`、border `.32`、字 muted ⇒ 太低调。
   改成实底色金 `rgba(245,179,66,.22)` + 粗体 600 + 7px 发光 dot + hover 光晕。
② **第二段置顶种子背景为白色（真 bug）**：`tr.sticky_top` 在真站**有两层** —— 外层(种子行本体 43 个)
   和**嵌套在 `<table class="torrentname">` 里的内层**(标题格里那条 43 个)。我的 `R + '.sticky_top'`
   用 `R = #torrenttable > tbody > tr:not(.colhead)` **直接子代选择器**, 只命中外层 ——
   内层 43 个**完全没有背景规则**, 露出站点默认白底。修: 加 `TN + '.sticky_top{background:var(--hdui-card)}'`
   (只加背景不加金边, 避免内外双金边)。真站探测: 86 个里有 43 深色 + 43 透明 ⇒ 修后 **86 全部深色**。
   **教训(P65): 嵌套表/嵌套行很常见, 同 class 规则不能只看外层; 检测方法是
   「DOM 选择器匹配数 ≠ 应有元素数」就追** —— 这条 86 匹配但只有 43 themed, 一眼看出漏了一半。
③ **搜索箱完全重新设计**: 站点 `table.searchbox` 是表格布局(每格 `td.embedded` 套 input/select/button),
   在深色主题上东拼西凑。改成 flex 卡片: 标题行金底金字 + 金底线、展开区 14px 内边距、
   控件统一深色圆角、提交按钮改金色。
   ⚠️ 关键: **不能用 `display:block !important`** —— 站点折叠机制靠 tbody 的内联
   `style="display: none"`(JS 切换), `!important` 会破坏折叠(sim `function-parity` 挂了才抓到)。
   改用 `:not([style*="display: none"])` 只在展开态铺卡片。
真站: 置顶 86 全深色; 搜索箱标题 `rgba(245,179,66,.06)` 底 + 金字、按钮金底;
八档/九档扫描全无。全量 **29/29**。
