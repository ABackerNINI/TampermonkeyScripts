...
## P48. 「原型」≠「真站」—— 照着设计稿写选择器，真站上会大面积打空

**背景**：HDHomeUI 的胶片墙原型是 `.workbuddy-ai/hdui-mock/film.html` —— 一个**手写的理想化页面**。
移植时把它的类名与结构当成了真站的样子，结果真站上有一整批样式**根本没生效**，
而仿真测试全绿（原因见 P49）。用户报「没能完整复刻原型」才查出来。

**逐条对照（左=原型，右=真站）**：

| 面 | 原型 | 真站（ground truth） | 后果 |
|---|---|---|---|
| 类别图标类名 | 7 个简化家族 | **12 族**：多 `c_cartoon_*` / `c_misc` / `c_4kuhd_remux` / `c_tvshows_*` / `c_tvmusics_*` | 未命中的那类**只被压尺寸、没被换图**，而 `catsprites.css` 是按 45×46 算的 `background-position` ⇒ 裁到 16×16 上是空白/碎片 |
| 促销标记 | `span.tags.tfree` | `<img class="pro_50pctdown\|pro_free\|pro_free2up">`（**没有 `tfree` 这个类**） | 样式一条也匹配不到 |
| 标签 | 与促销混为一谈 | `span.tags` 是**另一套东西**：`float:left;height:16px` + 23 种分类底色 | 完全没样式 |
| 列头单元格 | 「发布者」只有图标 | **该格自带文字** `<td class="colhead"><a>发布者</a></td>` | 脚本再补一遍 ⇒ 渲染成「发布者 发布者 ↓」 |
| 站点默认 CSS | 只有 `background` | 还有 `td.colhead{color:#fff;font-weight:bold}`、`table.torrents td.rowfollow{text-align:center}` | 列头是白粗字、标题被居中（脚本没重置） |
| 内层结构 | 2 格（标题 + RSS） | **3 格**（标题 / 豆瓣IMDb+下载收藏 / RSS） | 摊平成 `inline` 后变一条乱流 |
| 行内样式 | 无 | 类别图带 `style="background-image:…"`、RSS 带 `filter:grayscale(100%)` | 换上去的彩色 SVG 被去色 |
| 通配规则 | 无 | `table{background-color:#bccad6}` 会命中**标题格与 meta 格里嵌套的小表** | 深色片基上冒出浅蓝块 |

**通用对策**：
1. **有真站存档就用真站存档**（`resources-do-not-track/` 的脱敏整页 + 它的 `*.css`）当 ground truth；
   没有就先抓一份。原型只用来定"目标观感"，**不用来定"页面长什么样"**。
2. **做差集**：把「真站 CSS 里的类名清单」与「脚本里出现的类名清单」对齐，漏的那些就是没覆盖的样式。
   类别/标签这种"前缀族"要按真站实际清单核对，**并留一条兜底规则**。
3. **盯住会继承/穿透的属性**：`text-align` / `color` / `font-weight` / `filter` / `float` / `background`。
   站点给 `td.colhead` 这类**格子自己**的声明，会压过从 `<tr>` 继承来的值 —— 必须显式重置。
4. **特异性碰撞不会报错**：`a:hover{color}` 被 `ul#nav li a{color}` 压死，悬停就静默失效。
   改完用 `getComputedStyle` 实测，别只看源码里有这条规则。
5. **`<img>` 是替换元素**：伪元素不渲染 ⇒ "带文字的促销药丸"用纯 CSS 做不到，只能给图形徽章。

## P49. 仿真剧本从「原型」派生 ⇒ 全绿但真站画错（**全绿只证明没回归**）

**症状**：`node tests/run-all.js` **26/26 全绿**，真站上却有一整批样式没生效（P48）。
...
原型 film.html ──移植──▶ src/HDHomeUI.user.js
      ...
      └──照抄简化版──▶ tests/lib/sim/hdhome-ui-page.js ──断言──┘
                        ✗ 没有一条线连到真站
```
仿真页是**照原型写的简化副本**（类别只用那 7 个家族、标签用 `tfree`、标题格只有 2 格、
站点 CSS 只有 `background`），于是"原型 → 脚本 → 仿真"三者自洽，**唯独与真站脱节**。

**纪律**：
1. **仿真剧本必须从真站脱敏页派生**（只保留 class / 结构 / 站点 CSS，凭据一律 `***`），
   不能从设计原型派生。
2. **仿真页的 `SITE_CSS` 必须是真站 CSS 的子集，不能是"大概像"** ——
   缺一条真站规则就等于给脚本留一个测不到的盲区。
3. **先改仿真、再看变红**：把仿真页补齐成真站子集后，测试应当**大面积变红**；
   逐条与差异清单对账（对不上的说明还有没发现的差异），然后再修脚本。
4. **换肤/样式类脚本的验收必须包含"真站脱敏页离线渲染 + 与原型并排截图"**，光跑仿真不算数。
5. **结论**：**"仿真全绿"只证明没有回归，不证明真站画得对。**

**配套的仿真基建坑**：站点剧本对未知路径是**兜底渲染整页 HTML** 的。
把 HTML 当成图片响应（`/static/*.gif` 之类），`<img>` 就是"加载失败" ——
浏览器改用 **alt 文字**排版，一个 16px 的图标会变成 83px 宽（"download" 那串字），
布局与几何断言全被带偏。**静态资源必须在渲染剧本之前拦下来，返回能解码的 1×1 透明图**。

## P50. 换图标「只换 content」不够：站点的 `background-image` 带 `!important`，雪碧图透在新图标底下

**症状**：`<img>` 的 `content:url(svg)` 明明生效了（computed `content` 是 SVG），但界面上看起来
"图标没换" —— 或者换上去的图形底下压着一块莫名的碎片/空白。

**原因**：真站是拿**背景图**画图标的，而且带了 `!important`：

```css
/* catsprites.css */
img[class*="c_"]{width:45px;height:46px;background-image:url(catsprites.png) !important;
                 background-position:-385px -8px}
img.download{width:16px;height:16px;background:url(png.png) -80px 0}
```

`content` 只换了 `<img>` 的**前景内容**，背景层完全没动。而清背景用的是
`background:rgba(...)` **简写** —— 它隐含的 `background-image:none` 是**普通声明**，
**压不过 `!important`**。于是 45×46 的雪碧图照旧画在 SVG 底下，background-position 又是
按 45×46 算的，裁到 22px 的图标上就是一块碎片。

**正确做法**：背景层必须**显式**清/换，且带上 `!important`：

```css
/* 清掉: 类别图标这类"站点用雪碧图"的 */
img[class*="c_xxx"]{content:url(svg);background-image:none !important;
                    background-color:rgba(类别色,.16);}
/* 换掉: 站点用 background 画的(下载/收藏), content 与 background-image 双写同一张 */
img.download{content:url(svg);background-image:url(svg) !important;background-size:contain;}
```

**判定口诀**：**图标"换了但看起来没换"，先量 `background-image`，别去改 content 或图标设计。**
（与 P36「单列图标缺失先量列宽」、P42「点不到先查固有尺寸」是同一族的排查顺序。）

## P51. 非种子页的「大面积白底」来自站点通配 `table{}` 底色，不是 `body`

**症状**：主题明明生效了（`data-hdui-theme=film`、种子表也画对了），但打开「我的」/ 论坛 /
详情页这类**没有种子表**的页面，整屏仍是浅色。

**原因**：`table{background-color:#bccad6}` 是 **(0,0,1) 通配**，命中页面里**每一张**表。
脚本原先只覆盖了 `table.mainouter` / `table.main` / `#torrenttable` 三处 ——
种子表之外的表格（我的 / 论坛 / 搜索框）全在漏这块浅蓝灰。
`body` 其实早就被盖住了，所以查 body 是查不出问题的。

**正确做法**：配一条**通配 table** 的覆盖，并且确认它不会伤到片基 ——
`#torrenttable table{background:transparent}`(1,1,2) 特异性更高，种子表内的内嵌表不受影响。
同理 `td.rowfollow`(#f4f4f9) / `td.colhead` 也要一并压深；站点用 `bgcolor` 属性画的亮色块
（顶部通知条那种大黄条）得用 `[bgcolor]` 属性选择器 + `!important` 才盖得住。
另外 `<html>` 也要铺底色：只给 body 上色时，短页面在 body 之下仍会露出 `<html>` 的白。

## P52. 摊平内嵌表（`<tr>` 改 flex + 去掉 `<br>`）会把竖排的动作按钮挤成一条 inline 流

**症状**：标题格里「豆瓣 / IMDb / 下载 / 收藏」四个链接糊成一坨，彼此几乎贴在一起。

**原因**：真站第二格是 `<table><tr>[td(豆瓣<br>IMDb), td(下载<br>收藏)]</tr></table>`
—— 靠 `<br>` 竖排。脚本为了"省一整行高度"把 `<tr>` 摊平成 `display:flex` 并隐藏所有 `<br>`，
竖排结构没了，四组内容全被塞进同一条 flex/inline 流，只剩 6px 的 gap。

**正确做法**：竖排要**显式用 `flex-direction:column` 重建**，而不是靠隐藏 `<br>` 顺带摊平：

```css
/* 两列各自竖排: 评分列(豆瓣/IMDb) | 操作列(下载/收藏) */
tr{display:flex;align-items:flex-start;gap:10px;}
td{display:flex;flex-direction:column;align-items:flex-start;gap:3px;}
br{display:none;}          /* 竖排由 flex 负责, <br> 留着只会多撑出行距 */
a{display:inline-flex;align-items:center;gap:4px;padding:2px 6px;border-radius:6px;…}
```

每个动作再各自成一个 chip（圆角微底 + hover 转金），彼此就有明确的边界，不会粘连。
**验收要量矩形**：同列的两个 chip 左边缘对齐且上下排列，两列左右分开，四个两两不重叠。

## P53. 「清 border 不清 background」= 白页签：站点 `ul.menu li a` 自带 1px 白边 + `#dedede` 底

**症状**：上妆后顶部导航（首页 / 论坛 / 种子 …）是一排浅灰白页签，深色片基上极刺眼。
用户原话「首页/论坛等标签仍是白色」。

**根因**：脚本里早有注释写明「站点给 `ul.menu li a` 画了 1px 白边 + `#dedede` 底」，
但只写了 `border:0`，**漏了 background**。而原型 `film.html` 里导航项**默认无底色**
（只有 `:hover` 的 `rgba(255,255,255,.055)`）—— 所以从原型派生的仿真页也**没有这条底**，
仿真永远测不出来。又一次 P49 形状的盲区。

**修法**：
```css
html[data-hdui-theme] ul#mainmenu li a{…;border:0;background:transparent;…}
```
`background:transparent` 必须**显式写**（不能指望继承：站点的 `#dedede` 是实实在在的一条声明）。
特异性够（本规则 (1,1,4) > 站点 `ul.menu li a` 的 (0,1,3)），不需要 `!important`。

**两条延展教训**：
1. **「清样式」要成对清**。看到 `border:0` 就问一句"同一条站点规则里还有没有别的视觉属性没清"
   （background / box-shadow / outline / background-image），漏一个就是一块原站色漏出来。
2. **漏白扫描必须补 alpha**。按 `rgb` 三个分量算亮度会把 `rgba(255,255,255,0.043)` 这种
   **刻意的 4% 白底 chip** 全报成漏白（一次性 400 条误报），必须 `alpha < 0.5 则跳过`。
   同时**大面积阈值会漏掉小件**：页签/按钮只有几十 px²，
   `area > 20000` 的过滤条件会让它们全部隐身 —— 所以扫描要**分大小两档**跑。
   真站验收脚本里的「小件近白」就是为这个加的，本次 15 个白页签全靠它抓出来。

## P54. 逻辑键 ≠ DOM class：列映射把 `time` 归到 `alive`，但图标选择器要写站点真实的 class

**症状**：6 个指标图标里偏偏「存活」这一个还挂着站点雪碧图（`content:normal`），
它的 5 个兄弟（comments / size / seeders / leechers / snatched）都换成了 SVG。

**根因**：`detectColumns()` 里有一行 `if (/\btime\b/.test(cls) || /存活/.test(hint)) return 'alive';`
—— 它把站点的 `time` **归一化成逻辑键 `alive`**，这是对的（列映射要跨站点稳定）。
但 `ICON_MET` 生成的 CSS 是 `img.{键名}`，而真站列头的 DOM 是 `<img class="time">`：
**只写 `img.alive`，这条规则一条都不命中**。

**修法**：`ICON_MET` 里 `alive` 与 `time` **都写**（同一个 clock 图形、同一个色）。
代价是一行重复，收益是"逻辑键归逻辑键、DOM class 归 DOM class"不会再串台。

**排查手法（可复用）**：真站整页跑「站点残留装饰」扫描 —— 凡是
`background-image` 是 `url(http…)`（不是 `data:`）的 `<img>`，都是还没接管的族。
本次一次性扫出 7 族：`sticky`(86) / `star` / `arrowup` / `arrowdown` / `inbox` / `sentbox` / `buddylist`
（后经补扫又加 `time` / `rss` / `plus`），全部是脚本**完全没碰**的站点图标。
⚠️ 扫描白名单要留 `tr.sticky_top` 的 `box-shadow`（那是我们自己的置顶金边，不是站点残留）。

**另一条**：这些族**不在 `#torrenttable` 里**（箭头与信箱在底栏 `td.bottom`），
选择器加 `#torrenttable` 前缀就一条都不生效 —— 图标规则的**作用域**要按元素实际位置定，不能一律抄同一前缀。

## P55. 深色主题最容易漏的一类：站点 `<font color>` 老式着色 —— 深蓝/暗红压在深色底上等于看不见

**症状**：底栏的分享率 / 魔力值 / 邀请 / 上传量 / 下载量 / 做种位 / 活跃全部「看不见」
（站点一律 `<font class="color_*">` + `#1900d1`），数据行里的红色标记同理（暗到 `#550000`）。
⚠️ **用户不会说"对比度低"，只会说"那个数怎么没了"** —— 靠截图很难定位，必须自动扫。

**根因**：主题管了 `#info_block` 与它的 `b` / `a`，但**没管站点自己用 `<font color>` 写死的颜色**。
这类老式着色是**元素自身的声明**，优先级高于任何继承，不显式覆盖就原样保留。

**修法**：
1. `font.color_*` 7 个类 → 各给一个色板里的可读色位（分享率金、魔力值琥珀、上传绿、下载红、
   邀请紫、做种位蓝紫、活跃青绿）。
2. 暗红 `<font color="#xxxxxx">` → **按值枚举**统一提亮成 `#e5705f`（保留"这是红色强调"的语义）。
   ⚠️ CSS 拿不到"站点原本那个色"（`currentColor` 是继承值，不是自身指定值），
   所以只能枚举 —— 漏一个值就漏一处暗字，靠扫描补。

**⚠️ 生成 CSS 时的坑**：7 个色**必须一条一色分开写**。图省事把 7 个选择器并成一条、
声明块里连写 7 个 `color`，**只有最后一个生效**（前 6 个被覆盖），7 个统计值会变成同一个色 ——
而这在语法上完全合法，静态检查看不出来。已在 `sim-hdui-film-fidelity` 加
「7 个统计值颜色两两不同」断言，并**验证过改坏会红**。

**⚠️ 静态断言要打在表上，不要打在拼接结果上**：这些 CSS 是运行时由 `INFO_COLORS` 拼出来的，
源码里**没有**字面串 `font.color_ratio{color:#f5b342}`，按拼接结果写正则必然误报（刚踩过）。

**排查手法（可复用）**：真站整页跑「对比度」扫描 —— 取**自带文本**的元素（直接子文本节点非空），
沿祖先找第一个不透明底色当有效背景，`|前景亮度 - 背景亮度| < 40` 就报。只报结构与颜色不报文本。
本次一轮扫出 24 处（6 类深蓝 + 一批暗红），修完剩 1 处（`#aa0000`，补进枚举表后清零）。

## P56. UA 渲染的界面（滚动条 / 下拉弹层 / 自动填充 / 选中文字）不在任何扫描的视野里

**症状**（都是"用的时候才发现"）：滚动条是浅色的；`<select>` 展开的列表是白的；
填过的输入框被 Chrome 刷成浅黄底；选中文字的高亮色刺眼；placeholder 灰得看不清；
登录页的密码框仍是白底（主题只写了 `input[type=text]`）。

**为什么测不出来**：前几轮加的四档扫描（漏白 / 小件近白 / 残留装饰 / 对比度）
**全都走 `document.querySelectorAll`** —— 只覆盖 DOM 元素。滚动条、下拉弹层、自动填充背景、
`::selection` 是**浏览器画的**，不是元素，扫描永远看不见，只能靠"源码里有这几条"来兜。

**修法（`.18`）**：
1. `html[data-hdui-theme]{color-scheme:dark;}` —— **这一条是主力**：浏览器会自己把滚动条、
   下拉弹层、日期选择这些 UA 界面画成深色。零几何改动。
2. 自动填充：`input:-webkit-autofill` 的浅黄底**改不了 `background-color`**（优先级高于普通声明），
   只能用 `-webkit-box-shadow: 0 0 0 1000px var(--hdui-bg) inset` 盖掉，并配 `-webkit-text-fill-color`。
3. `::selection` / `::placeholder` / `:focus-visible` 各给主题色。
4. 输入框选择器要覆盖 `password` / `number` / `email` / `url` / `search` / `tel`，别只写 `type=text`。

**⚠️ 不要自己写 `::-webkit-scrollbar{width:...}`**（实测踩过）：
- 把默认 15px 压成 10px ⇒ 内容区宽 5px ⇒ 站点那套固定宽布局跟着长 5px ⇒
  文档宽 **1247 → 1252**，`sim-hdui-layout-width` 的「不撑宽页面」直接红。
- 而且**任何** `::-webkit-scrollbar-*` 规则都会让 Chrome 从 overlay 切回 classic 滚动条，同样是几何改动。
- 结论：**主题不该改 UA 的几何，只该改它的颜色** —— 着色交给 `color-scheme`。
  已加反断言「源码不得出现 `'html[data-hdui-theme] ::-webkit-scrollbar` 规则」防回退。

**⚠️ 反断言的匹配要带规则前缀**：只搜 `::-webkit-scrollbar{` 会被源码注释里
"不要写 ::-webkit-scrollbar{width...}"这句话**自己命中**（刚踩过）。要搜 `'html[data-hdui-theme] ::-webkit-scrollbar`。

## P57. CSS 的失败是**静默**的：规则写错会被浏览器直接丢弃，不报错也不渲染

**为什么这是本项目的头号隐患**：HDHomeUI 是**纯 CSS 层** —— 全部效果靠往 `style#hdui-css`
里塞规则。而 CSS 出错时浏览器是**静默丢弃整条规则**：不抛异常、不渲染、页面上只是"没变化"。
前几轮加的四档扫描（漏白 / 小件近白 / 残留装饰 / 对比度）看的都是**渲染结果**，
"这条规则根本没生效"这类问题它们照样发现不了。

**典型死法**：`TN_META + '{flex:0 0 auto'` 少一个 `}`；`statStack` 生成 `:nth-child(undefined)`；
`content:url(...)` 里 SVG 没转义；选择器拼出 `td[[[bad` 这种语法错误。

**检查手法（已固化成 `sim-hdui-css-validity.js`，第 29 个用例）**：
把 `style#hdui-css` 的文本按**顶层 `{}`** 切成一条条，逐条喂给一个临时 `<style>`，
用 CSSOM 的 `cssRules.length` 判断有没有被解析 —— CSSOM 会直接丢掉无法解析的规则，
所以「切出来 N 条」与「解析出 M 条」的差就是死规则，且能把原文打印出来定位。

真站实测：**251 条切出来、251 条解析、0 条丢弃**。
已**反向验证过**：故意塞一条 `html[data-hdui-theme] td[[[bad-selector{...}`，用例立刻红并把原文报出来。

**注意区分**：**无效的属性值不会让规则整条消失**（CSS 只忽略那一条声明、保留规则），
所以这个检查只抓「选择器/括号/语法」层面的错误，抓不到 `color:#zzz` 这种 —— 后者要靠渲染扫描。

## P58. 「同一条列网格」必须片头与数据行**横向 padding 一致** —— 差 1px 就是整行错位

**症状**（.19 用户实拍「标题栏错位 / 标题栏没对齐」）：12 列的列头**全部**比数据往左 10px。
真站逐列量出来 `Δl=10`（每一列都差 10px，不是单列错位）。

**根因**：本主题把表格改成了 flex 布局 ——
- 片头 `H`：`padding:10px 0 9px`（左右 **0**）
- 数据行 `R`：`padding:7px 10px`（左右 **10px**）

片头和数据行是**各自独立的 flex 行**，列宽由 flex 决定；横向 padding 不一致 ⇒
两条行的**内容起点**差 10px，12 列一起错。这不是 table-layout 的问题，是**两条 flex 行自己的内边距不等**。

**修法**：片头改成 `padding:10px 10px 9px`，与数据行的 10px 对齐。实测 `Δl: 10 → 0`。

**排查手法（可复用）**：真站整页逐列量 `表头单元格.left - 数据单元格.left`，
再看两个 `<tr>` 自身的 boundingRect —— 若 **tr 一致但 cell 差 N**，就是行的横向 padding 差 N；
若 **tr 本身就差 N**，那是别的（比如 margin / 不同的 flex 容器）。
本次先误以为是 `border-collapse` 或 `table-layout`（试了都不相关），量出 cell 差 10、tr 一致后才定位到 padding。

**防回退**：`sim-hdui-film-fidelity` 加「片头与数据行逐列对齐（12 列左右边缘差 ≤ 2px）」断言，
并**反向验证过**改回 `padding:10px 0 9px` 会红（12 列全报 `dL:10`）；`check-hdui-static` 加两条源码断言。

## P59. 促销徽章分色：三种促销**同色**＝用户分不出哪个是 Free

**症状**（.19 用户实拍「种子缺少Free标记」）：真站 28 个 `pro_free` **全都渲染了**
（可见 28、content→SVG 28），但用户坚称"缺少 Free 标记"。

**根因**：三种促销徽章**全是同一个金色 `#f5b342`** ——
`pro_free`（免费）/ `pro_50pctdown`（5折）/ `pro_free2up`（免费+双倍上传）长得一模一样，
用户没法从一堆金色徽章里认出哪个是 Free。

**修法**：按语义分色 —— 免费 = 绿 `#7fb84e`（PT 通行语义）、5折 = 琥珀 `#f5b342`、
免费+双倍 = 紫 `#9b7fd4`。判据沿用导航 16 色的同一套（色相/饱和/亮度不能同时接近）。

**教训**：**"元素渲染了" ≠ "用户看得出"**。以后报"缺标记/缺图标"先查**是不是撞色/太小/太像**，
不要先假设没渲染 —— 用探针量「可见数 / content→SVG 数 / 宽高」就能区分这两种情况。

## P60. 进度指示器的英文替换：CSS 换不了文字，只能动叶子 text 节点

**症状**（.19 用户实拍「下载进度显示不全（将 Leeching 改为中文）」）：
`<td bgcolor="#CC0066"><font color="#fff"><b>0% <br> Leeching </b></font></td>`
—— "Leeching" 在窄格里截断，且是英文。

**为什么不能纯 CSS**：CSS 只能换样式，**换不了 text 节点里的一个词**。
若要纯 CSS 只能整段 `<b>` 隐藏再 `::before` 补，但那会把百分比一起吞掉（数据丢失）。
所以这是脚本里**唯一一处直接改站内文本**的逻辑 —— 只动叶子 text 节点里的单词，
不改 DOM 结构、不挂事件、不动样式属性；只在 theme-on 时调一次。

**⚠️ 两个坑**：
1. **单词不是 `firstChild`**！结构是 `<b>{pct}% <br> {word} </b>`，`firstChild` 是 `"{pct}% "`，
   单词在**第二个** text 节点 —— 必须遍历所有子节点，否则一个都换不掉（刚踩过）。
2. **进度指示器有两种、颜色也不同**：
   ...
   只按 `font[color="#fff"]` 选会漏掉做种那种 —— 按 **`td[bgcolor] b`** 选，两种都覆盖
   （真站实测 4 个单元：3 做种 + 1 下载，全部替换）。

## P61. 「显示不全」要做成通用扫描：文本截断（scrollW/H > clientW/H）

**起因**：.20 用户实拍「下载进度显示不全」—— 我按描述去改，但意识到**这类问题只能等用户截图**。
于是加了「文本截断」扫描（真站整页）：
凡是 `scrollWidth - clientWidth > 1` 或 `scrollHeight - clientHeight > 1` 的元素都报，
**排除** `text-overflow:ellipsis`（那是刻意省略，不算 bug）。只报结构与几何值，不报文本。

**立刻抓到一处我没注意到的**：`<td class="colhead">进度` 被裁 **3px** ——
列宽 40px，但列头内容是 `::before` 图标 15px + gap 5px + 文字「进度」~23px = **43px**。
数据行的"100% / 做种中"两行也挤。把 `FILM_W.progress` 40 → **48** 解决。

**关键点**：`FILM_W` 这张表是**片头与数据行共用**的同一套 `flex-basis`（见 `basis` 数组分别 push 到 H 和 R），
改一个值两边同时变 —— 所以列宽类问题**只改这一处**就够了，不要去分别调片头/数据行。

**判据小结（遇到"显示不全/对不齐"）**：
- 逐列量 `cell.left` 差 → 是列宽/padding 问题
- 量 `scrollWidth vs clientWidth` → 是容器太窄（内容被裁）
- 两者都看：本次 padding 差 10px 与列宽少 3px 是**两个独立问题**，一次扫描各抓一个。

## P62. 列宽类规则必须**片头 + 数据行两边都写** —— 只写一边会在窄屏错位（宽屏看不出来）

**症状**（.21 多视口复验时发现）：1024 视口下列 [1]（标题）表头 256px、数据行 240px，
后面 11 列**整体左移 16px**；1280 / 1366 视口下却完全对齐。

**根因**：主题里 `tdSel(m,k)` 生成的是 **`R > td…`（只数据行）**，"片头"要用 `H > td…`。
标题列的 `max-width:max(240px, calc(100vw - 900px))` 原来只写在数据行 ⇒
- 1024 视口：上限 `max(240, 124)` = **240**，可用 **256** ⇒ 数据行被卡在 240、片头长到 256 ⇒ 差 16
- 1280 视口：上限 `max(240, 380)` = 380 > 可用 256 ⇒ 两边都是 256 ⇒ **看不出问题**

**修法**：新增 `hSel(m,k)`（片头列选择器），把这条 `max-width` **同时给两边**。

**教训**：
1. **片头与数据行是两条独立的 flex 行**，任何影响列宽的规则（flex-basis / min-width / max-width /
   padding）**必须两边都写**，只写一边就会错位。已有 `FILM_W` 那套 flex-basis 是两边都 push 的
   （`basis` 数组分别 push 到 H 和 R），所以要特别注意**后来单独加的规则**有没有漏掉片头。
2. **窄屏才暴露的 bug 要在多个视口复验**。这次 1280（默认）测了 3 轮都没发现，
   是加了 `HDUI_VW` 视口参数跑 1024 才暴露的 —— 单个视口的"全绿"不代表对齐是真的。

## P63. 压平站点底色会**连状态线索一起压掉** —— 记得用文字/字色补回来

**起因**：`.14` 加了 `td[bgcolor]{background:var(--hdui-panel)}` 抹掉站点的亮色块（那时是为了治
「顶部通知条亮黄块」）。但**进度指示器也是靠底色区分状态的**：
...
⇒ 两种状态长得一模一样，**只剩文字能分辨**。这是"修 A 问题顺手把 B 的线索弄丢"的典型。

**为什么漏了这么久**：`#CC0066` 的亮度约 **73**，`#44cef6` 约 **161** ——
漏白扫描的门槛是 **> 200**，两者都**不在扫描范围内**，所以扫不出来。
（教训：**"不是白底"≠"没问题"**，中等亮度的语义色块要单独量。）

**修法**（`.23`）：底色压平后，用**文字颜色**把线索补回来 ——
`translateLeeching()` 在替换文字时顺便给格子打 `data-hdui-prog="seed"|"leech"` 标记，
CSS 按标记染字色（做种=绿 `#7fb84e` / 下载=琥珀 `#f5b342`）。

**两个实现要点**：
1. **认原词，别认替换后的中文** —— 标记要在**替换之前**根据 `Seeding`/`Leeching` 判定，
   否则重复调用（结构守卫会重试）时中文已经不是英文了，认不出来。
2. **必须连 `font` / `b` 一起染** —— 站点在 `<font color="#fff">` / `<font color="#FF0066">` 上
   写死了自己的 `color`，只染 `td` 会被子元素的声明盖掉（又一次"子元素声明压过继承"）。
3. 用 `data-*` 标记而不是 `td[bgcolor="#44cef6"]` 选择器：
   **认词语比认站点配色稳**，站点改配色不会连带弄坏这个。

## P64. 「此刻可见」的扫描抓不到 **hover 态 / 隐藏态** 的元素 —— 要主动按选择器查

**起因**：加了「中间带亮块」扫描（背景 lum 60~200，补 P63 的盲区）后，扫出
`div.niceTitle[domTT__autoId1]` —— 站点 **DomTT 提示框**，背景 `#7c98ae`（亮蓝灰）+ 黑字。

**为什么之前所有扫描都没发现**：它平时是 `visibility:hidden`（`opacity:0`），
hover 才显示。而漏白 / 小件近白 / 对比度 / 文本截断**全都是「扫此刻可见的元素」**
⇒ 隐藏的元素永远不在视野里，但它**一 hover 就在深色底上亮一块**。

**修法**：深色化 `div.niceTitle` 和 `div.smilies td`（论坛表情面板，同一个 `#7c98ae`）。

**教训（重要）**：
- **渲染扫描的覆盖面 = "此刻可见"**，hover 态 / 弹层 / 提示框 / 折叠面板 **一律测不到**。
- 这类元素只能**主动按选择器查**：在真站上 `querySelector` 出来，直接读它的
  `background-color` / `color`（哪怕它此刻是 hidden），判断有没有被主题覆盖。
- 判据：读到的是**站点原色**（`#7c98ae` 这类）就说明漏了；读到主题色（`rgb(25,21,18)`）才对。

**白名单要写清理由**，否则每次扫描都报：
- `span.tags` 的 23 种分类色 —— TASK019 用户拍板**刻意保留**（深色底上唯一彩色元素）
- 主题金 `rgb(245,179,66)` —— 当前页高亮 / 做种数这类**刻意的强调色**

## P65. 「同一条 tr.sticky_top」要分两层 — 内层(标题格嵌套表)会漏底

**症状**(.25 用户实拍"第二段置顶种子背景为白色"):
真站 100 行里 86 行是 `tr.sticky_top`, 但其中 **43 行是嵌套在 `<table class="torrentname">` 里的内层 tr**(标题格里那条),
还有 43 行是**外层** `tr`(种子行本体)。我的 `R + '.sticky_top{box-shadow:...}'`
用 `R = #torrenttable > tbody > tr:not(.colhead)`, **直接子代选择器**,
只命中外层那 43 行 —— 内层那 43 行**完全没有背景规则**, 露出站点默认**白底**。

**真站探测证实**: 86 个 tr.sticky_top, 父级全在 `<tbody>` 下(没有更深的嵌套),
但 86 个里有 43 个 `background: rgb(32,28,24)`(深色, themed)、43 个 `rgba(0,0,0,0)`(透明)。
比例正好 1:1 —— 内外两层各一半。

**修法**: 加一条 `TN + '.sticky_top{background:var(--hdui-card);}'`
(`TN = #torrenttable table.torrentname > tbody > tr`), **只**给内层加背景不加金边
(避免内外双金边)。修后: 86 个全部 `rgb(32,28,24)`。

**教训**:
1. **嵌套表/嵌套行很常见** —— 任何「同 class」规则都不能只看外层。
   有两种防御: ① 用更宽的选择器(`#torrenttable tr.sticky_top`, 不限子代),
   ② 给每个**实际可能存在的层级**都写一条。② 更稳但啰嗦, ① 容易误伤。
2. **检测"漏规则"**: 探测时**按元素数 vs 父级分布**, 发现「同 DOM 选择器匹配数 ≠ 应该有的元素数」就追。
   这条: 86 个 tr.sticky_top 但只有 43 个 themed ⇒ 立刻知道"还有 43 个没拿到规则"。

## P66. 「看不见」不等于「display:none」—— 四类隐蔽的屏蔽通道

**背景**(2026-09-20 用户诉求): 要保证主题**不屏蔽未知元素**(消息弹窗 / 公告 / 站点新增的种子标签)。
先去数 `display:none` 是没用的 —— 全脚本只有 3 条对外隐藏声明(两个 `<br>` + 搜索箱折叠小图标),
真正的风险全在"元素还在、矩形非零、人却看不见"的通道里。这四类**此前一条测试都没有**:

| 通道 | 机理 | 触发条件 |
|------|------|----------|
| 图标被清成空白 | `background-image:none !important` 之后没有补任何 content | 站点新增 `pro_*` / `c_*` 家族(枚举清单里没有) |
| 前景色没管 | 主题只压深**背景**, 不管站点**写死的前景色** | 公告块 `<font color="#000000">`、`style="...;color:#000"` |
| 继承到站点黑字 | 站点 `table.torrents{...color:#000}`, 主题没重置 | 种子表里**任何没有显式上色**的新元素 |
| contain / overflow / z-index | 改定位基准、裁掉溢出、压住浮层 | 表内 `position:fixed` 弹窗、格内新角标、低 z-index 浮层 |

**实测抓到的两条真 Bug**(都是被新用例 `sim-hdui-unknown-canary` 逼出来的):
1. **`table.torrents{color:#000}` 从未被重置** —— 主题只写了 `background/border/padding`。
   已上色的元素(标题链接 / 数值列 / `td[data-hdui-prog]` / `font.color_*`)都是**自己的声明**, 所以一直没暴露;
   但站点将来在种子表里加的任何新徽章都会**继承到纯黑**, 压在深色片基上就是隐形。
   修: `#torrenttable{...color:var(--hdui-fg);}`(继承值压不过直接声明, 不伤既有着色)。
2. **促销徽章没有兜底** —— `PRO_BADGES` 是**按名枚举**的, 而 filmCss 有一条通配的
   `#torrenttable img[class*="pro_"]{width:16px;height:16px;background:none;}` 只清底不换图。
   站点新增的 `pro_xxx` 于是变成 16×16 的空白/雪碧图碎片。修: 在枚举之前补一条兜底
   (`iconUri('medal', PRO_FALLBACK_COLOR)`), 与类别图标同一套纪律(**兜底必须排在具体家族之前**)。

**教训**:
1. **"屏蔽"的判据不能只看 DOM 与矩形** —— 必须加上 hit-test(防被裁/被压)、对比度(防隐形)、
   图标 content/background 至少一层是内联 SVG(防空白)三道。
2. **阈值取 40 而不是 20**: 纯黑(亮度 0)压在片基(亮度 22)上亮度差正好 22 ——
   取 20 会把它判成"合格", 而它正是最典型的软屏蔽。与真站对比度扫描同一口径。
3. **反例必须逐个跑**: 写完全部断言后, 把每条生产兜底删掉一次, 确认对应的断言真的会变红。
   本轮 8 条反例里有 2 条一开始是绿的(判据写得太松), 就是靠反例发现的。

## P67. inline style 的兜底要**按写法枚举**, 漏一种写法就漏一块

**症状**: 主题有 `[style*="background:#fff"]` 兜底, 却没有 6 位写法 `background:#ffffff`。
公告条 / 跑马灯最常写 6 位 ⇒ 底没被压深, 留一块白。

**为什么两条扫描都抓不到**: 漏白扫描看的是**背景亮度**, 对比度扫描看的是**前景 vs 背景** ——
白底 + 黑字的对比度是**满分**, 两条都判"没问题"。只有人眼能看出"这里有一块白"。
⇒ **"看得见" ≠ "没问题"**: 白底本身就是漏网, 哪怕它上面的字看得清。

**修法**: 把 `background` / `background-color` × `#fff` / `#ffffff` / `white` / `rgb(255,255,255)` ×
带空格/不带空格 **全列一遍**(与 .14 同一套路), 新增一类写法时同步补。
**教训**: 按值枚举的兜底, 每补一种写法都要写进 `check-hdui-static.js` 的 INLINE_OK 清单, 否则会悄悄退化。

## P68. 行结构必须**逐行**校验 —— 只查 `rows[1]` 会放过"补列补到一半"

**症状**: `detectColumns` 原来只比 `body.rows[1].cells.length !== head.cells.length`。
外部脚本补 A / A·GB 两列是**逐行插**的, 第一行补完、后面还没补的那一刻被判成"结构正常"
⇒ 10 列的行被硬塞进 12 列的槽位, **整张表列全错位**, 而结构守卫的等窗口逻辑根本不会启动。

**修法**: 改成 `for (let i = 1; i < body.rows.length; i++)` 逐行比对。
**测试抓手**: 新增仿真剧本 `hdhome-ui-laterow`(只有第 6 条数据行少一格) ——
旧行为下这个剧本会**照样上妆**, 新行为下会进等窗口再回退。反例已验证。

**教训**: 「抽样第一条」在"逐行渐进式改动"面前永远是错的 —— 校验要覆盖**全集**, 不是代表元。

## P69. 未知结构一律卸妆(2026-09-20 用户裁决)

**裁决**: 站点新增**认不出来的列**(`E_COLUMN_UNEXPECTED`)或**认不出来的行**
(带 colspan 的分组/公告行, `E_ROW_UNKNOWN`)时, 主题**不猜**, 直接回退原站界面。

**为什么不是"照常上妆"**: 主题是「每列一个 `nth-child` 槽位」的排版。多出来的列/行没有槽位,
硬上妆的结果只有两种 —— 被摆到错误的槽位, 或被 `overflow:hidden` 压成一坨。**两者都等于屏蔽未知元素**,
与"对未知元素不默认屏蔽"直接冲突。认不出来就把页面还给原站, 反而是最守底线的做法。
...
## P72. 不可证伪的断言比没有断言更危险 —— 反例要跑"删掉被测代码"才算数

**怎么发现的**: 顺着 `sim-security-s04-favicon-scheme.js` 偶发失败去查, 顺手做了反例 ——
把 `collectFavicon` 里的 `if (!/^https?:/i.test(abs)) return;`(伪协议过滤)**删掉**,
S04 **依然全绿**。

**为什么**: 那行后面还有一道同站约束 `isSameSiteHost(iconHost, unitHostOf(u))` ——
`javascript:` / `data:` 的 `hostname` 是空串, **永远不可能"同站"**, 所以第二道照样挡得掉。
协议过滤在当前实现里只是**纵深防御的第二道**, 而 S04 的注释却宣称它在测第一道。
它真正能抓的只有"两道一起被删"这种退化。

**后果**: 这条用例长期给人一种"伪协议过滤有端到端覆盖"的错觉 ——
**红灯会骗人, 一直不红也会骗人**。

**三条纪律**:
1. **反例必须"删掉被测的那段代码", 而不是"改一点让它看起来不对"** ——
   删了还绿, 说明这条断言在当前实现下不可证伪, 要么补一个能证伪的场景, 要么如实标注覆盖率。
2. **不可证伪的用例要在文件头写明"能抓住什么、抓不住什么"**(已写进 S04 的头注释),
   否则下一个人会继续误信它。
3. **"固定 sleep 再读存储"的负向断言尤其危险**: 机器慢时脚本还没跑到,
   存储是空的 ⇒ 断言通过 ⇒ **什么都没验**(假绿)。改成轮询一个**完成信号**
   (PTAC 里是 `#ptac-root-v2` 挂载 —— `collectFavicon()` 在 `main()` 第一行, UI 在其后)。
   S03 那条**正向**断言同理(慢机器会误报成"没采集到")。

## P73. 顺带修掉的: S03/S04 用固定 sleep 代替轮询
