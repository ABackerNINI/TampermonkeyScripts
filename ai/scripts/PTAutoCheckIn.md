...
> 源文件：`src/PTAutoCheckIn-v2.user.js` ｜ 版本 `2026.09.07.7`(升级时同步更新)
...
7. 新增 UI 交互: 点击站点名 → 新标签打开该站; 失败且冷却中的站被排除且行有特殊样式(琥珀竖条+降透明度); 常规批量按钮右侧 `^` 展开 → 琥珀色「强制批量签到」按钮(带 tooltip), 点它把失败冷却站纳入并强制点击(注意反爬风险, 谨慎使用); 全部完成/批量中 `^` 隐藏。
