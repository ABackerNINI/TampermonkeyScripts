...
- FAB(右下角, 渐变圆钮)→ 点击展开面板：今日汇总 + 分组列表(多吧 group 有组头)+ 每行状态徽章/msg/时间 + 「批量签到(剩余 n)」按钮。**FAB 皮肤 4 套可切换**(面板头部「外观」展开选择条, 存 `ptac_skin` 跨站生效): ①变色 全完成→绿底✓(无角标)/ 有未签→紫底✓+红底剩余角标; ②数字 全完成→✓ / 有未签→主区大字剩余数(绿角标=成功数); ③信号灯 完成→绿✓ / 今日有失败→红底!+失败角标 / 其余→琥珀✓+剩余角标; ④光环 主体保持紫底, 外圈渐变弧(青→品红, 带辉光呼吸动效)按完成比例推进; 全完成→绿满环; 今日有失败→琥珀→红警示弧; 绿角标=成功数。核心诉求: **全部签到完毕与有站点未签到在 FAB 上有明显区别**。
...
- **失败且冷却中的站**(今日 `failed` + 10min 冷却未过): 整行降透明度 + 左侧琥珀竖条 + 副文案提示「冷却中, 默认不参与批量」; 批量候选**默认排除**此类站。常规「批量签到」按钮右侧有 **`^` 展开钮** → 点击展开显示琥珀色的 **「强制批量签到(n 站)」** 按钮(带详细 tooltip): 无视冷却点击强制重试(点击前仍重写冷却, 再失败进入新一轮冷却)。**全部签到完毕(无失败冷却站)或批量进行中不显示 `^`**; 强制按钮带详细 tooltip 说明适用场景与风控风险。
...
7. 新增 UI 交互: 点击站点名 → 新标签打开该站; 失败且冷却中的站被排除且行有特殊样式(琥珀竖条+降透明度); 常规批量按钮右侧 `^` 展开 → 琥珀色「强制批量签到」按钮(带 tooltip), 点它把失败冷却站纳入并强制点击(注意反爬风险, 谨慎使用); 全部完成/批量中 `^` 隐藏; 站点图标: 各站名前显示 favicon(路过收集 → 根路径兜底 → 加载失败隐藏, 不影响布局)。
